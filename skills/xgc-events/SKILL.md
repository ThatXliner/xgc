...
```bash
xgc events <resource> <method> [flags]
```
...
| Command | Description |
|---------|-------------|
| [`+subscribe`](../xgc-events-subscribe/SKILL.md) | Subscribe to Workspace events and stream them as NDJSON |
| [`+renew`](../xgc-events-renew/SKILL.md) | Renew/reactivate Workspace Events subscriptions |
...
xgc schema events.<resource>.<method>
```
