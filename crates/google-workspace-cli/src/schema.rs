...
/// Handles the `xgc schema <dotted.path>` command.
///
    ...
    if parts.len() == 2 {
        ...
        if let Some(schema) = doc.schemas.get(schema_name) {
            ...
            if doc.resources.contains_key(schema_name) {
                return Err(GwsError::Validation(format!(
                    "'{schema_name}' is a resource. To see its methods, try 'xgc schema {service_name}.{schema_name}.list' (or similar). To see a type definition, try 'xgc schema {service_name}.<Type>'."
                )));
            }
